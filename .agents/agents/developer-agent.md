...
- Ready tasks from `.agents/memory/tasks.yaml`.
- Requirements and acceptance criteria from `.agents/docs/requirements.md`.
- Architecture guidance from `.agents/docs/architecture.md`.
- Test strategy from the Test Engineer Agent.
- Review feedback from the Code Review Agent.

## Outputs

- Production code under `.agents/src/`.
- Tests under `.agents/tests/`.
- Minimal documentation updates when behavior changes.
- Progress and status updates in `.agents/memory/tasks.yaml` and `.agents/memory/progress.md`.
