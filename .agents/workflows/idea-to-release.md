...
1. Product Requirements Agent captures the idea in `.agents/docs/requirements.md`.
2. Product Requirements Agent defines users, goals, non-goals, constraints, and acceptance criteria.
3. Project Coordinator Agent creates tasks in `.agents/memory/tasks.yaml` and marks implementation-ready items as `ready`.
4. System Architect Agent updates `.agents/docs/architecture.md` and records major decisions in `.agents/memory/decisions/`.
...
9. DevOps Agent runs CI/release checks and updates `.agents/docs/release-checklist.md`.
10. Project Coordinator Agent marks tasks `done`, updates `.agents/memory/progress.md`, and logs follow-up work.
